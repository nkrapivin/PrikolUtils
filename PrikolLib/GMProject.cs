...
namespace PrikolLib
{
    ...
    {
        public const int GMMagic = 1234321;
        ...
        private void PostLoad()
        {
            ...
            for (int i = 0; i < Objects.Count; i++)
            {
                var gmobject = Objects[i];
                if (gmobject == null) continue;

                gmobject.PostLoad(this);
                for (int j = 0; j < gmobject.Events.Count; j++)
                {
                    var gmevent = gmobject.Events[j];
                    for (int k = 0; k < gmevent.Count; k++)
                    {
                        var e_in = gmevent[k];
